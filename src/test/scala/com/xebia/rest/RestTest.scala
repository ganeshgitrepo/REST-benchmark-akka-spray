...
class RestTest extends Specification with SprayTest with RestService with DontDetach with TestKit with BeforeAfter {
	doBeforeSpec {
		MemoryHashStore.storeActor.dispatcher = Dispatchers.globalExecutorBasedEventDrivenDispatcher
		MemoryHashStore.storeActor.start()
	}
	val recordStore = MemoryHashStore

	doAfterSpec {
		MemoryHashStore.storeActor.stop()
	}
